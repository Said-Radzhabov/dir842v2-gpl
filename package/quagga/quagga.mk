...
define ENABLE_LDPD
	$(APPLY_PATCHES) $(@D) $(PKGDIR)/ldpd-patches \*.patch || exit 1
endef
QUAGGA_POST_PATCH_HOOKS += ENABLE_LDPD
endif
...
define QUAGGA_CLEANUP_TMP
	$(Q)rm -f $(TARGET_DIR)/tmp/ripd.conf.sample
	$(Q)rm -f $(TARGET_DIR)/tmp/ripngd.conf.sample
	$(Q)rm -f $(TARGET_DIR)/tmp/zebra.conf.sample
endef
QUAGGA_POST_INSTALL_TARGET_HOOKS += QUAGGA_CLEANUP_TMP
