...
missing_progs="no"
for prog in patch perl tar wget rsync bc ${DL_TOOLS} ; do
	if ! which $prog > /dev/null ; then
...
# зависимости для запаковки broadcom прошивки
if grep -q -E '^BR2_PACKAGE_BCM_HOTTOOLS=y' $BR2_CONFIG ; then
    required_perl_modules="${required_perl_modules} Convert::Binary::C"
    required_perl_modules="${required_perl_modules} Digest::CRC"
    check_prog_host "xxd"
fi
