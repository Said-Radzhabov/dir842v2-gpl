...
BR2_LOCALVERSION := $(shell $(TOPDIR)/scripts/setlocalversion)
...
BUILD_TIMESTAMP := $(TOPDIR)/.build_time
export DATE_EPOCH := $(shell test -f $(BUILD_TIMESTAMP) && cat $(BUILD_TIMESTAMP) || sh -c "date +%s | tee $(BUILD_TIMESTAMP)")
...
target-finalize: $(TARGETS) $(TARGET_DIR) gen_hashes
...
ifeq ($(BR2_HAVE_DEVFILES),y)
	...
	$(Q)rm -rf $(TARGET_DIR)/usr/lib/cmake
	$(Q)rm -rf $(TARGET_DIR)/usr/include $(TARGET_DIR)/usr/lib/pkgconfig $(TARGET_DIR)/usr/share/aclocal $(TARGET_DIR)/usr/share/bash-completion/
	$(Q)rm -rf $(TARGET_DIR)/include $(TARGET_DIR)/lib/pkgconfig $(TARGET_DIR)/share/aclocal $(TARGET_DIR)/share/bash-completion/
...
target-post-image: $(TARGETS_ROOTFS) target-finalize $(TARGETS_PACK)
	$(Q)rm -f $(BUILD_TIMESTAMP)
...
show-targets:
	@echo $(sort $(TARGETS)) $(sort $(TARGETS_ROOTFS))
...
COMMON_CONFIG_ENV = \
	...
	KCONFIG_JSON=$(BUILD_DIR)/buildroot-config/auto.json \
